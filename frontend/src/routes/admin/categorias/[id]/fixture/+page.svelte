<script lang="ts">
	...
	async function cargarDatos() {
		...
		error = '';
		try {
			const [fixtureRes, posRes, catRes] = await Promise.all([
						...
						{#if !heat.completado}
							{#if activeHeat === heat.id}
								<div class="result-form">
									<input type="text" bind:value={ordenInput} placeholder="auto-id-1, auto-id-2, auto-id-3, auto-id-4" class="input" />
									<div class="btn-row">
										<button class="btn btn-sm" onclick={() => { activeHeat = null; ordenInput = ''; }}>Cancelar</button>
										<button class="btn btn-sm btn-primary" onclick={() => registrarResultado(heat.id)} disabled={actionLoading}>Registrar</button>
									</div>
								</div>
							{:else}
								<button class="btn btn-sm btn-primary" onclick={() => activeHeat = heat.id}>Registrar Resultado</button>
							{/if}
						{/if}
					</div>
				{/each}
